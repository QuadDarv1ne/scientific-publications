...
class StarlinkPrometheusExporter:
    ...
    def _define_metrics(self):
        ...
        # Performance metrics
        self.download_mbps = Gauge("starlink_download_mbps", "Download speed in Mbps")
        self.upload_mbps = Gauge("starlink_upload_mbps", "Upload speed in Mbps")
        self.ping_ms = Gauge("starlink_ping_ms", "Ping latency in milliseconds")
        self.packet_loss_percent = Gauge("starlink_packet_loss_percent", "Packet loss percentage")

        # Starlink-specific metrics (if available)
        self.snr = Gauge("starlink_dish_snr", "Signal to Noise Ratio")
        self.obstruction_fraction = Gauge(
            "starlink_dish_fraction_obstruction_ratio", "Fraction of sky obstructed"
        )
        self.downlink_throughput_mbps = Gauge(
            "starlink_dish_downlink_throughput_mbps", "Dish downlink throughput in Mbps"
        )
        self.uplink_throughput_mbps = Gauge(
            "starlink_dish_uplink_throughput_mbps", "Dish uplink throughput in Mbps"
        )

        # Monitoring metrics
        self.scrape_duration_seconds = Histogram(
            "starlink_scrape_duration_seconds", "Time to scrape metrics from database"
        )
        self.scrape_counter = Counter("starlink_scrape_total", "Total number of metric scrapes")
        self.up = Gauge("starlink_up", "Was the last scrape successful (1 = success, 0 = failure)")

        # Info metric
        self.info = Info("starlink_exporter", "Information about the Starlink exporter")
        self.info.info({"version": "1.0.0", "project": "starlink_performance_monitor"})
...
def main():
    ...
    parser.add_argument(
        "--config", default="config.json", help="Configuration file path (default: config.json)"
    )
    parser.add_argument(
        "--port", type=int, default=9817, help="HTTP port to expose metrics (default: 9817)"
    )
    parser.add_argument(
        "--interval", type=int, default=3, help="Metrics update interval in seconds (default: 3)"
    )

    args = parser.parse_args()

    try:
        exporter = StarlinkPrometheusExporter(config_path=args.config, port=args.port)

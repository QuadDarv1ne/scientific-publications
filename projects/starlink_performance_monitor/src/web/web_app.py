...
from src.web.realtime_manager import start_realtime_updater
...
def get_latest_metrics():
    ...
    try:
        ...
        latest_metric = (
            session.query(PerformanceMetric).order_by(PerformanceMetric.timestamp.desc()).first()
        )
        session.close()
        ...
        try:
            ...
            if db.query(User).filter((User.username == username) | (User.email == email)).first():
                return render_template(
                    "register.html", error="Пользователь с таким именем или email уже существует"
                )
            # Create user
            ...
            db.commit()
            return render_template(
                "register.html", message="Аккаунт создан. Теперь вы можете войти."
            )
        except Exception as e:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not user:
                ...
                return render_template(
                    "forgot_password.html", message="Если такой email существует, ссылка отправлена"
                )
            # Generate token
...
def reset_password(token: str):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if password != confirm:
                return render_template(
                    "reset_password.html", valid=True, error="Пароли не совпадают"
                )
            if len(password) < 6:
                return render_template(
                    "reset_password.html",
            ...
            return render_template(
                "reset_password.html",
                valid=False,
                message="Пароль обновлён. Теперь вы можете войти.",
            )
...
def api_ml_analysis():
    ...
    try:
        ...
        analysis = analyzer.generate_report(
            30, 90, 7
        )  # Anomaly days, forecast days, prediction days

...
from src.database.db_manager import (  # noqa: E402  # импорт после настройки sys.path
    get_database_manager,
    get_db_session,
)
from src.database.models import (  # noqa: E402  # импорт после настройки sys.path
    Base,
    WeatherData,
)
from src.utils.logging_config import get_logger  # noqa: E402  # импорт после настройки sys.path
...
class WeatherDataCollector:
    ...
    def get_weather_data(
        ...
        try:
            ...
            # Get parameters
            parameters = weather_config.get(
                "parameters", ["temperature_2m", "precipitation", "wind_speed_10m", "cloud_cover"]
            )

            self.logger.info(
                f"Fetching weather data for {latitude}, {longitude} from {start_date_str} to {end_date_str}"
            )
    ...
    def _store_weather_data(
        self, weather_data: List[Dict[str, Any]], latitude: float, longitude: float
    ):
        """
    ...
    def correlate_with_performance(
        self, weather_data: Dict[str, Any], performance_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Starlink Performance Monitor Weather Data Collector"
    )
    ...
    if weather_data:
        ...
        print(
            f"Location: {weather_data['location']['latitude']}, {weather_data['location']['longitude']}"
        )

        # Show first few records
        print("\nFirst 5 weather records:")
        for _i, record in enumerate(weather_data["data"][:5]):
            print(f"  {record}")
    else:

...
from src.utils.logging_config import get_logger, setup_logging  # Configure logging

setup_logging(
    config_file=os.path.join(os.path.dirname(__file__), "..", "utils", "logging_config.json")
)
...
class ReportGenerator:
    ...
    def generate_performance_chart(self, report_data: Dict[str, Any], filename: str):
        ...
        axes[0, 0].plot(
            df["timestamp"], df["download_mbps"], label="Download Speed (Mbps)", color="blue"
        )
        axes[0, 0].plot(
            df["timestamp"], df["upload_mbps"], label="Upload Speed (Mbps)", color="green"
        )
        ...
        # Plot 3: Packet Loss
        axes[1, 0].plot(
            df["timestamp"], df["packet_loss_percent"], label="Packet Loss (%)", color="orange"
        )
        ...
        axes[1, 1].hist(
            df["download_mbps"], bins=20, alpha=0.7, label="Download Speed", color="blue"
        )
    ...
    def generate_correlation_heatmap(self, report_data: Dict[str, Any], filename: str):
        ...
        for i in range(len(numeric_cols)):
            for j in range(len(numeric_cols)):
                plt.text(j, i, f"{corr_data.iloc[i, j]:.2f}", ha="center", va="center", fontsize=12)
...
def main():
    ...
    parser.add_argument(
        "--format", choices=["json", "csv", "both"], default="json", help="Output format"
    )
    ...
    if args.type == "daily":
        ...
        default_output = (
            f"custom_report_{start_date.strftime('%Y-%m-%d')}_{end_date.strftime('%Y-%m-%d')}"
        )

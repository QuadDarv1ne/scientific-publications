...
from sqlalchemy import desc  # noqa: E402  # импорт после настройки опциональных зависимостей

from src.database.db_manager import (  # noqa: E402  # импорт после настройки sys.path
    get_database_manager,
    get_db_session,
)
from src.monitor.monitor import PerformanceMetric  # noqa: E402  # импорт после настройки sys.path
...
class Alert:
    ...
    def __init__(self, alert_data: Dict[str, Any]):
        ...
        self.last_notification = alert_data.get("last_notification")
...
class EnhancedAlertSystem:
    ...
    def check_thresholds(self) -> List[Alert]:
        ...
        try:
            latest_metric = (
                session.query(PerformanceMetric).order_by(desc(PerformanceMetric.timestamp)).first()
            )
    ...
    def _check_metric_thresholds(
        self, metric: PerformanceMetric, thresholds: Dict[str, float]
    ) -> List[Dict[str, Any]]:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Starlink Performance Monitor Enhanced Alert System"
    )

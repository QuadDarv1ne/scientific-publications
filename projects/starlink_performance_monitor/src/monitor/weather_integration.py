...
class WeatherPerformanceAnalyzer:
                ...
                performance_data.append(
                    {
                        ...
                        "packet_loss_percent": float(metric.packet_loss_percent)
    ...
    def get_weather_data(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            weather_records = (
                ...
                .filter(WeatherData.timestamp >= start_date, WeatherData.timestamp <= end_date)
                ...
                weather_data.append(
                    {
                        ...
                        "temperature_2m": float(record.temperature_2m)
                        if record.temperature_2m is not None
                        else None,
                        "precipitation": float(record.precipitation)
                        if record.precipitation is not None
                        else None,
                        "wind_speed_10m": float(record.wind_speed_10m)
                        if record.wind_speed_10m is not None
                        else None,
                        "cloud_cover": float(record.cloud_cover)
                        if record.cloud_cover is not None
                        else None,
                        "pressure_msl": float(record.pressure_msl)
                        if record.pressure_msl is not None
                        else None,
                        "humidity_2m": float(record.humidity_2m)
                        if record.humidity_2m is not None
                        else None,
                    }
                )
    ...
    def analyze_correlations(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Analyzing correlations for {len(performance_data)} performance records and {len(weather_data)} weather records"
            )
            ...
            return {
                ...
                "data_points": {"performance": len(performance_data), "weather": len(weather_data)},
    ...
    def _calculate_correlations(
        self, weather_data: List[Dict[str, Any]], performance_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
                    ...
                    for perf_param in performance_params:
                        if perf_param in merged_df.columns:
                            ...
                            if len(clean_data) > 1:
                                ...
                                correlations[weather_param][perf_param] = (
                                    float(corr) if not pd.isna(corr) else 0.0
                                )
                            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Starlink Performance Monitor Weather-Performance Correlation Analysis"
    )
    ...
    if report:
        ...
        print(
            f"Data Points - Performance: {report['data_points']['performance']}, Weather: {report['data_points']['weather']}"
        )
        print()
        ...
        if correlations:
            ...
            for weather_param, perf_correlations in correlations.items():
                ...
                for perf_param, details in perf_correlations.items():
                    print(
                        f"    {perf_param}: {details['correlation']:.3f} ({details['interpretation']})"
                    )
        else:

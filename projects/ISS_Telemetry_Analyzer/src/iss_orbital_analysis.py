...
class ISSTracker:
    ...
    def get_current_position(self):
        ...
        try:
            ...
            if data.get("message") == "success":
                ...
                # Валидация координат
                if DataValidator.validate_coordinates(position["latitude"], position["longitude"]):
                    logger.info(
                        f"Положение получено: {position['latitude']:.4f}, {position['longitude']:.4f}"
                    )
                    return position
                else:
        ...
        try:
            ...
            for pos in self.positions:
                if (
                    ...
                    and "timestamp" in pos
                    and isinstance(pos["timestamp"], datetime)
                ):
    ...
    def analyze_altitude_trend(self, save=True, show=True):
        ...
        try:
            ...
            for i, _day in enumerate(time_days):
            ...
            plt.plot(
                ...
                "r--",
                linewidth=2,
                label=f"Тренд (наклон: {trend_slope:.3f} км/день)",
            )
            ...
            plt.title(
                "Анализ тренда изменения высоты орбиты МКС", fontsize=14, fontweight="bold", pad=15
            )
            plt.legend(loc="upper right", fontsize=11, framealpha=0.9)
            plt.grid(True, alpha=0.3, linestyle="--")
            ...
            plt.text(
                0.02,
                0.98,
                f"Начальная высота: {initial_altitude:.1f} км",
            ...
            plt.text(
                0.02,
                0.92,
                f"Средняя высота: {np.mean(altitude):.1f} км",
                transform=plt.gca().transAxes,
                fontsize=11,
                verticalalignment="top",
                bbox={"boxstyle": "round", "facecolor": "white", "alpha": 0.8},
            ...
            plt.text(
                0.02,
                0.86,
                f"Тренд: {trend_slope * 1000:.1f} м/день",
                transform=plt.gca().transAxes,
                fontsize=11,
                verticalalignment="top",
                bbox={"boxstyle": "round", "facecolor": "white", "alpha": 0.8},
    ...
    def get_real_time_data(self):
        ...
        try:
            ...
            orbital_params = (
                ...
                else {"altitude_km": 408.0, "orbital_period_min": 92.9, "inclination": 51.64}
            )
...
def predict_passes(latitude, longitude, n_passes=5):
    ...
    for i in range(n_passes):
        ...
        print(
            f"  {i + 1}. {pass_time.strftime('%d.%m.%Y %H:%M')} | "
...
def analyze_pass_frequency(latitude, longitude, days=7):
    ...
    try:
        ...
        print(
            f"📊 Среднее количество пролетов в день: {avg_passes_per_day:.1f} ± {std_passes_per_day:.1f}"
        )
        print(f"🔺 Максимум пролетов в день: {max_passes}")
        print(f"🔻 Минимум пролетов в день: {min_passes}")
        print(
            f"🌟 Наиболее активный день: День {most_active_day + 1} ({passes_per_day[most_active_day]} пролетов)"
        )
        print(
            f"🌑 Наименее активный день: День {least_active_day + 1} ({passes_per_day[least_active_day]} пролетов)"
        )
        ...
        plt.title(
            f"Частота пролетов МКС над точкой ({latitude}°, {longitude}°)",
            fontsize=14,
            fontweight="bold",
            pad=15,
        )
        ...
        plt.axhline(
            y=avg_passes_per_day,
            color="red",
            linestyle="--",
            linewidth=2,
            label=f"Среднее: {avg_passes_per_day:.1f}",
        )
        plt.legend()

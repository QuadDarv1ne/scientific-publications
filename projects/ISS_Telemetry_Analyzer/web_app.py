...
def current_position():
    ...
    try:
        # Симуляция данных
        position = {"latitude": 51.6, "longitude": -123.4, "timestamp": datetime.now().isoformat()}
...
def radiation_analysis():
    ...
    try:
        ...
        return jsonify({"success": True, "image": img_str, "total_dose_mSv": float(total_dose_mSv)})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def pass_frequency():
    ...
    try:
        ...
        ax.bar(days_range, passes_per_day, color="skyblue", alpha=0.7, edgecolor="navy")
        ax.set_xlabel("Дни")
...
def real_time_data():
    ...
    try:
        ...
        external_temp = (
            40 + (121 - 40) * np.sin(np.pi * orbital_phase / 0.6) if is_sun_side else -157
        )

        # Симуляция радиации
        base_radiation = 30
        radiation = (
            base_radiation * (2 + 2 * np.random.rand())
            if (current_time.minute % 10) < 3
            else base_radiation
        )
        ...
        return jsonify({"success": True, "data": real_time_data})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})

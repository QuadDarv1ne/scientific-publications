...
class CMEDetector:
    ...
    def track_cme(self, lasco_data: List[SolarImage]) -> List[CME]:
        ...
        for i in range(len(lasco_data) - 1):
            ...
            if np.any(bright_regions):
                ...
                cme = self._extract_cme_parameters(img1, img2, bright_regions)
    ...
    def _extract_cme_parameters(
        self, img1: SolarImage, img2: SolarImage, mask: np.ndarray
    ) -> Optional[CME]:
        """
        ...
        direction_lon = np.arctan2(centroid[1] - center[1], centroid[0] - center[0]) * 180 / np.pi

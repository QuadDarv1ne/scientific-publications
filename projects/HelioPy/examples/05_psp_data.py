...
from heliopy import load_psp_fld, load_psp_sweap
...
def main():
    ...
    try:
        ...
        print(
            f"  Плотность: {spc_data['density'].mean():.2f} ± {spc_data['density'].std():.2f} частиц/см³"
        )
        print(
            f"  Скорость: {spc_data['velocity'].mean():.2f} ± {spc_data['velocity'].std():.2f} км/с"
        )
        print(
            f"  Температура: {spc_data['temperature'].mean() / 1e6:.2f} ± {spc_data['temperature'].std() / 1e6:.2f} MK"
        )
        print()

...
def proc_frame_reader_and_detection(
    queue_out: Queue, config: DictConfig, time_sleep_start: int, stop_event: Event
) -> None:
    """
...
def proc_tracker_update_and_calc(
    queue_in: Queue, queue_out: Queue, config: DictConfig, stop_event: Event
) -> None:
    """
...
def proc_show_node(queue_in: Queue, config: DictConfig, stop_event: Event) -> None:
    """
    ...
    try:
        ...
        processes: List[Process] = [
            ...
            Process(
                ...
                args=(
                    queue_frame_reader_and_detect_out,
                    queue_track_update_out,
                    config,
                    stop_event,
                ),

# Development Setup Script for Traffic Analyzer
...
Write-Host "║  TRAFFIC ANALYZER - DEV ENVIRONMENT SETUP        ║" -ForegroundColor Cyan
Write-Host "╚═══════════════════════════════════════════════════╝`n" -ForegroundColor Cyan
...
if (-not $allConfigsOk) {
    Write-Host "`n⚠️  Некоторые конфигурационные файлы отсутствуют" -ForegroundColor Yellow
    Write-Host "   Скачайте веса модели: https://github.com/ultralytics/assets/releases/download/v8.2.0/yolov8m.pt`n" -ForegroundColor Cyan
}

# Создание .env из примера
if (-not (Test-Path ".env") -and (Test-Path ".env.example")) {
    Write-Host "`n📝 Создание .env из .env.example..." -ForegroundColor Yellow
    Copy-Item ".env.example" ".env"
    Write-Host "  ✓ .env создан. Отредактируйте его при необходимости`n" -ForegroundColor Green
}
...
Write-Host "  • Форматирование:      " -NoNewline; Write-Host "black ." -ForegroundColor White
Write-Host "  • Проверка типов:      " -NoNewline; Write-Host "mypy ." -ForegroundColor White
Write-Host "  • Запуск тестов:       " -NoNewline; Write-Host "pytest" -ForegroundColor White
Write-Host "  • Локальный запуск:    " -NoNewline; Write-Host ".\test-local.ps1`n" -ForegroundColor White

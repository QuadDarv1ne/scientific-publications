...
class KafkaProducerNode:
    ...
    def __init__(self, config) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Kafka Producer инициализирован. Topics: {self.topic_name}, {self.metrics_topic}"
            )
    ...
    def process(self, frame_element: FrameElement):
        ...
        if current_time - self.last_send_time > self.how_often_sec or frame_element.frame_num == 1:
            ...
            traffic_data = {
                ...
                "cars": frame_element.info["cars_amount"],
                "road_1": (
            ...
            try:
                ...
                self.logger.debug(
                    f"Отправлены метрики производительности в топик {self.metrics_topic}"
                )

...
class TestEnhancedNotificationSystem(unittest.TestCase):
    ...
    def test_should_notify_for_pass_basic(self):
        ...
        should_notify = notifier.should_notify_for_pass(
            "STARLINK-1234", max_elevation=45.0, brightness=1.0
        )
    ...
    def test_notify_upcoming_pass_filtering(self):
        ...
        with (
            patch.object(notifier, "send_email_notification", return_value=True) as mock_email,
            patch.object(
                notifier, "send_telegram_notification", return_value=True
            ) as mock_telegram,

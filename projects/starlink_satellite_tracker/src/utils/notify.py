...
class NotificationSystem:
    ...
    def send_email_notification(self, subject: str, message: str, recipient: str) -> bool:
        ...
        try:
            ...
            if not subject or not message or not recipient:
                self.logger.error(
                    "Invalid email parameters: subject, message, and recipient are required"
                )
                return False
    ...
    def send_pushover_notification(self, message: str, title: str = "Starlink Tracker") -> bool:
        ...
        try:
            ...
            data = {"token": api_token, "user": user_key, "message": message, "title": title}
    ...
    def should_notify_for_pass(
        ...
        try:
            ...
            if max_elevation < min_elevation:
                self.logger.debug(
                    f"Skipping notification for {satellite_name}: Elevation {max_elevation}° below minimum {min_elevation}°"
                )
                return False
            ...
            if brightness < min_brightness:
                self.logger.debug(
                    f"Skipping notification for {satellite_name}: Brightness {brightness} below minimum {min_brightness}"
                )
                return False
            ...
            if velocity > 0 and velocity < min_velocity:
                self.logger.debug(
                    f"Skipping notification for {satellite_name}: Velocity {velocity} km/s below minimum {min_velocity} km/s"
                )
                return False
            ...
            if satellite_name in excluded_satellites:
                self.logger.debug(
                    f"Skipping notification for {satellite_name}: Satellite is excluded"
                )
                return False
            ...
            for pattern in excluded_patterns:
                if pattern in satellite_name:
                    self.logger.debug(
                        f"Skipping notification for {satellite_name}: Matches excluded pattern '{pattern}'"
                    )
                    return False
            ...
            if included_satellites and satellite_name not in included_satellites:
                self.logger.debug(
                    f"Skipping notification for {satellite_name}: Satellite not in included list"
                )
                return False
            ...
            if included_patterns:
                ...
                if not pattern_match:
                    self.logger.debug(
                        f"Skipping notification for {satellite_name}: Does not match any included pattern"
                    )
                    return False
...
"""
            ...
            if self.email_config.get("enabled", False):
                ...
                if recipient:
                    email_success = self.send_email_notification(
                        "Starlink Satellite Pass Alert", message, recipient
                    )
            ...
            if self.pushover_config.get("enabled", False):
                pushover_success = self.send_pushover_notification(
                    message, "Starlink Satellite Pass"
                )
...
def create_notification_example():
    ...
    try:
        ...
        success = notifier.notify_upcoming_pass(
            "STARLINK-1234", datetime.now() + timedelta(minutes=45), 65.5, 42.3
        )

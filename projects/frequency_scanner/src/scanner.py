...
class FrequencyScanner:
    ...
    def scan_range(
        self, start_mhz: float, end_mhz: float, step_mhz: float = 1.0, dwell_seconds: float = 0.5
    ) -> list[dict]:
        """
        ...
        while current_mhz <= end_mhz and self.is_scanning:
            ...
            scan_points.append({"frequency": current_mhz, "level": level})
    ...
    def scan_preset(self, preset: str = "all", dwell: float = 0.3) -> list[dict]:
        ...
        bands = self.BANDS.keys() if preset == "all" else [preset]
        ...
        for band in bands:
            if band in self.BANDS:
                ...
                results.extend(self.scan_range(start, end, step_mhz=0.5, dwell_seconds=dwell))

        return results

    def record_frequency(
        self, freq_mhz: float, duration: int = 30, output_format: str = "wav"
    ) -> str | None:
        """
...
def main():
    ...
    parser.add_argument(
        "--range", nargs=2, type=float, metavar=("START", "END"), help="Диапазон в MHz"
    )
    ...
    parser.add_argument(
        "--preset",
        choices=["all", "fm", "air", "vhf", "uhf", "pmr", "lpd"],

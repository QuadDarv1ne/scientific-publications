...
class SimpleSystemMonitor:
    ...
    def get_network_info(self):
        ...
        try:
            ...
            if hasattr(self, "last_net_io") and hasattr(self, "last_net_time"):
                ...
                upload_speed = (
                    (net_io.bytes_sent - self.last_net_io.bytes_sent) / time_diff / 1024
                )  # KB/s
                download_speed = (
                    (net_io.bytes_recv - self.last_net_io.bytes_recv) / time_diff / 1024
                )  # KB/s
            else:
                    ...
                    try:
                        ...
                        for gpu in c.Win32_VideoController():
                            if gpu.Name:
                                gpu_info["GPU_0"] = {
                                    "name": gpu.Name,
                                    "memory": f"{int(gpu.AdapterRAM) / (1024**3):.1f} GB"
                                    if gpu.AdapterRAM
                                    else "N/A",
                                }
                    except:
    ...
    def display_dashboard(self, data):
        ...
        print(
            f"Система: {sys_info['platform']} {sys_info['platform_release']} ({sys_info['architecture']})"
        )
        print(f"Хост: {sys_info['hostname']}")
        print(
            f"Аптайм: {int(sys_info['uptime'] // 3600)}ч {int((sys_info['uptime'] % 3600) // 60)}м"
        )
        ...
        cpu = data["cpu"]
        print(
            f"CPU: {cpu['usage']:5.1f}% | Ядра: {cpu.get('cores', 'N/A')} | Частота: {cpu.get('frequency', 'N/A')}"
        )
        ...
        if "error" not in mem:
            print(
                f"ОЗУ: {mem['percent']:5.1f}% | Используется: {mem['used']:5.1f}GB / {mem['total']:5.1f}GB"
            )
        ...
        if "GPU_0" in gpu:
            gpu0 = gpu["GPU_0"]
            print(
                f"GPU: {gpu0.get('load', 0):5.1f}% | Память: {gpu0.get('memory_percent', 0):5.1f}% | {gpu0.get('name', 'N/A')}"
            )
        ...
        for disk_name, disk_info in list(disks.items())[:3]:  # Показываем первые 3 диска
            if "error" not in disk_info:
                print(
                    f"{disk_name}: {disk_info['percent']:5.1f}% | {disk_info['used']:5.1f}GB / {disk_info['total']:5.1f}GB"
                )
